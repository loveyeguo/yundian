...
namespace CommonLibrary
{
    ...
    public class SQLiteHelper
    {
        ...
        {
            string strCon = "Data Source=" + Application.StartupPath + "\\" + dbname + ";Pooling=true;";
            this.sqliteConnectionString = strCon;
        }
        public SQLiteHelper()
        {
            // string strCon = "Data Source=" + Application.StartupPath + "\\examdb.db;Pooling=true;Password=yanc;"
            string strCon = "Data Source=" + Application.StartupPath + "\\Microsoft.Excels.dll;Pooling=true;";
            this.sqliteConnectionString = strCon;
        }
        public void Jiami()
        {
            ...
            connection.Open();
            connection.ChangePassword("yanc");

        }
        public void Jiemi()
        {
            SQLiteConnection connection = new SQLiteConnection(sqliteConnectionString);
            connection.SetPassword("yanc");
            connection.Open();
        ...
        public int ImportFromDataTable(DataTable dt)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(sqliteConnectionString))
            {
                ...
                using (DbTransaction transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        //先删除数据库再导入数据
                        command.CommandText = "delete from " + dt.TableName;
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            values = "null," + values.TrimEnd(',');
                            command.CommandText = "insert into " + dt.TableName + " values (" + values + ")";
